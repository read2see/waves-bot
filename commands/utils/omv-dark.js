...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('omv-d')
		.setDescription('Posts NMS Waves Old man version dark theme.'),
	async execute(interaction) {
		...
		let fullURL = origin + screenshotsLinks.omv_dark;
		const imageAttachment = {
			attachment: fullURL,  
			name: extractNameFromURL(fullURL)
		}
		try{
			await interaction.editReply({content: 'Recent Old man version - dark:', files:[imageAttachment]})
			console.log('\n----------------Recent Old man version - dark screenshot sent.')
		}catch(error){
