...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('omv-l')
		.setDescription('Posts NMS Waves Old man version light theme.'),
	async execute(interaction) {
		...
		let fullURL = origin + screenshotsLinks.omv_light;
		const imageAttachment = {
			attachment: fullURL,  
			name: extractNameFromURL(fullURL)
		}
		try{
			await interaction.editReply({content: 'Recent Old man version - light:', files:[imageAttachment]})
			console.log('\n----------------Recent Old man version - light screenshot sent.')
		}catch(error){
